...
import { 
  ...
  ExternalLink, Search, Settings2, Loader2, Activity
} from "lucide-react";
...
export default function IntegrationsPage() {
  ...
  const handleConnection = async (app: any) => {
    ...
    try {
      ...
      const msg = err.response?.data?.error?.message || "Failed to initialize connection";
      toast(msg, "error");
    }
  };
            ...
            <Card className="border-border/50 hover:shadow-xl hover:shadow-slate-200/50 transition-all group rounded-2xl overflow-hidden">
              <CardHeader className="pb-4">
                <div className="flex items-start justify-between">
                  ...
                  <Badge variant={app.status === "Connected" ? "default" : "outline"} className={`rounded-lg ${app.status === 'Connected' ? 'bg-emerald-500 hover:bg-emerald-600 text-white border-none' : 'text-slate-400'}`}>
                    {app.status}
                  </Badge>
                </div>
